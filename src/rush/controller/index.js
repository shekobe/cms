...
export default class extends Base {
  ...
  async indexAction(self){
    ...
    //购买条件不正确
    if((parseInt(obj_param.saleCount<1) || parseInt(obj_param.saleCount>5)) || obj_param.goodsId == '' || isNaN(obj_param.goodsId) || isNaN(obj_param.saleCount)){
    ...
    // let current_count = 0;
    // try{
    //   current_count = (self.current_count)?self.current_count: await self.getredis('graprush_buy_'+obj_param.goodsId) || 0;
    // }catch(err){
    //   console.log(' 当前可卖数目',err);
    // }
    // if( parseInt(current_count) >= parseInt(self.sale_count) ){
    //   self.current_count = self.sale_count;
    //   self.disconnectredis();
    //   return self.errors('soldout')
    // }
    ...
    try{
      self._redis.set("graprush_userqueue_"+this.token,user_num+'_'+Date.now());
    }catch(err){
        ...
        try{
          ...
          for(let j=0;j<goodsTimearr.length;j++){
            ...
            if(currentTimemap < parseInt(vallist[1])){
              str += new Date(parseInt(vallist[0])).toLocaleDateString() + ' '+new Date(parseInt(vallist[0])).toLocaleTimeString()+'至'+new Date(parseInt(vallist[1])).toLocaleDateString() + ' '+new Date(parseInt(vallist[1])).toLocaleTimeString()+',';
              // break;
            }
          }
  ...
  async createconnectredis(){
    ...
    try{
      ...
      if(think.env == 'development'){//开发环境
        ...
        this._redis = new ioRedis(ioredis_config.product);
      }
    }catch(err){
