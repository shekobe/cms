 ...
 * @author <teen hewendong@360shouji.com>
 * @created 20150210
 */
;(function($) {
	$.fn.carousel = function(options) {
		var defaults = {
			...
			speed: 500, //动画时间
			loop:false,  //是否循环
			pagination:false, //是否有计数机,计数器的选择器
			paginationon:'on',//计数器hover,默认为on
			scrollnum: 4, //同时滚动个数
			itemview: 4, //显示个数
		...
		return this.each(function() {
			...
			function _scroll(type){
				...
				if(opt.direction == 'y'){
					...
					//返回当前index
					if(opt.pagination){
						var index = Math.abs(Math.floor(result/scrollY));
						$(opt.pagination).children().eq(index).addClass(opt.paginationon).siblings().removeClass(opt.paginationon);
					} 
					...
					//
					if(opt.pagination){
						var index = Math.abs(Math.floor(result/scrollX));
						$(opt.pagination).children().eq(index).addClass(opt.paginationon).siblings().removeClass(opt.paginationon);
					} 
			...
			// next
			$(opt.clickWarp).delegate(opt.next,'click', function(){
			...
			//pre
			$(opt.clickWarp).delegate(opt.prev,'click', function(){
				if(!opt.clickloop){//不循环点击滚动
				var marginTop = parseInt(_this.find(opt.itemWarp).css('margin-top')),
					marginLeft = parseInt(_this.find(opt.itemWarp).css('margin-left'));
