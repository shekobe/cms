               ...
               // form.submit(); // form validation success, call ajax form submit
                ...
                $.ajax({
                    ...
                    data:{
                        ...
                        remember:$('.login-form').find('input[name="remember"]').attr('checked'),
                        password:$('.login-form').find('input[name="password"]').val(),
                        qktoken:$('input[name="qktoken"]').val()
                    },
                    cache: false,
                    beforeSend:function(){
                        $('.login-form').find('input[type="submit"]').addClass('disabled');
                    },
                    success:function(data){
                            if(data.errno == 0){//返回数据正常
            ...
            submitHandler: function(form) {
                $.ajax({
                    ...
                    data:{
                        ...
                        power:$('.register-form').find('input[name="role"]:checked').val(),
                        qktoken:$('input[name="qktoken"]').val()
                    },
                    cache: false,
                    beforeSend:function(){
                        $('.register-form').find('input[type="submit"]').addClass('disabled');
                    },
                    success:function(data){
                        if(data.errno == 0){//返回数据正常

                            if(data.data == 'username-taken'){
                                bootbox.alert('用户名已经存在，请重新填写');
                            }else{
                    ...
                    complete:function(xhr, textStatus){
                        if(xhr.status != '200')
                             bootbox.alert('后台数据异常');                          
                       $('.register-form').find('input[type="submit"]').removeClass('disabled'); 
                    }
                });
